...
const widget = {
  ...
  mappings: {
    series: {
      endpoint: "series",
      map: (data) => ({
        wanted: jsonArrayFilter(data, (item) => item.monitored && item.statistics.episodeCount > item.statistics.episodeFileCount).length,
        have: jsonArrayFilter(data, (item) => item.hasFile || (item.statistics && item.statistics.episodeFileCount > 0)).length,
        missing: jsonArrayFilter(data, (item) => item.monitored && item.statistics.episodeCount > item.statistics.episodeFileCount).length,
